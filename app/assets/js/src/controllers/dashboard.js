teechioDashboard.controller('DashboardController', [ '$scope', '$route', '$http', '$acache', function DashboardController($scope, $route, $http, $acache) {
	...
	$scope.createApplication = function() {
		$http.post('/account/app', $scope._app)
		...
		.error(function(err) {
			var clazz = 'error';
			if(err.error == 'not_allowed_plan') clazz = 'warning';
			$scope.$emit('nprogress:loading', { state: 'done' });
			$scope.$emit('ux:notification', { container: '#new-application', clazz: clazz, msg: err.message });
		});
	};

	$scope.loadApps = function() {
		$http.get('/account/apps')
	...
	$scope.loadStats = function() {
		...
		$http.get('/account/stats/storage')
		.success(function(res, status) {
	...
	$scope.loadSettings = function() {
		...
		.success(function(res, status) {
			...
			var quotas = { calls: 50000, storage: 1024 };
